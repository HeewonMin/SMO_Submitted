...
Info.Localidx = [1]; % Design Variable 중에서 Local variable 이 몇 번째 index 에 해당하는가 

Info.Coupledidx = [2]; % Design Variable 중에서 Coupled variable 이 몇 번째 index 에 해당하는가 
Info.CoupledLagidx = [1]; % Lagrange multiplier 중에서 몇 번째 index 에 해당하는가
...
Info.LocalVar = [5]; % Design Variable 중에 Local variable.
Info.LocalVarStd = [0.5]; % PATC 에서 쓰일 Local variable 의 std 값.
Info.LocalVarType = {'Normal'}; % PATC 에서 쓰일 Local variable 의 분포타입.

Info.Var = [5 5]; % Variable 의 initial value 값.

%% Output 부분에서 Coupled Variable 부분
Info.LinkingPerfidx = []; % FEA analysis 중에 linking variable 의 index.
Info.LinkingLagidx = []; % Lagrange multiplier 중에서 몇 번재 index 에 해당하는가?

Info.LinkingInVar = [5]; % 다른 subsystem 으로부터 들어오는 Linking Variable 중 여기서는 Variable 로 사용되는 값.
Info.LinkingInPerf = []; % 다른 subsystem 으로부터 들어오는 Linking Variable 중 여기서는 Performance 로 사용되는 값.
...
Info.Obj = @(x) x(1)+x(2); % 목적함수; % 목적함수.
Info.FEAFile = 'FEA_subsystem2'; % FEA 함수
...
SubsystemInfo{2} = Info; % 여기를 subsystem 에 대해서 숫자 바꿔준다.
